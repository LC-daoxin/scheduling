<template>
	<view class="wyb-table-box" @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend">
			...
			<view class="wyb-table-header" :style="{borderBottom: '1px solid' + borderColor}">
				...
				<view ref="iosBug" class="wyb-table-header-item" v-for="(item, index) in headers" :key="item.key" @tap="onHeaderItemTap(index)"
				 :style="{
					 ...
					 <view v-if="index !== 0" class="day" :style="{marginLeft: autoSortShow(index) && textAlign !== 'left' ? fontSize[0] * 0.65 + 'rpx' : 0}">
	...
	export default {
		...
		methods: {
			touchstart (event) {
				...
				this.touchStartPointX = event.touches[0].pageX;
				this.touchStartPointY = event.touches[0].pageY;
				this.distanceX = 0;
				this.distanceY = 0;
			},
			touchmove (event) {
				console.log(event)
				this.touchMovePointX = event.touches[0].pageX;
				this.touchMovePointY = event.touches[0].pageX;
				this.distanceX = this.touchMovePointX - this.touchStartPointX
				this.distanceY = this.touchMovePointY - this.touchStartPointY
